...
X_means = torch.load(f'{int_path}/{dataset_prefix}means.pt', weights_only=False)
...
for params in permutations_dicts:
    ...
    model_config = {'name':models.GRUD, 'params':{'input_size':len(data_columns), 'cell_size':len(data_columns),
                                        'hidden_size':len(data_columns), 'X_mean':X_means, 
                                        'device':device, 'zero_iter':59, 'output_last':True}}    # 59, 18, 13
